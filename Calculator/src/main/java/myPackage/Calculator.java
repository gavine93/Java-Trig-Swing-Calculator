/*
 Pseudocode for PostFix method referenced from https://en.wikipedia.org/wiki/Shunting-yard_algorithm
 Pseudocode for negative number support referenced from user Braids Constance in his comment from this post: https://stackoverflow.com/questions/40553697/how-can-i-accept-negative-values-in-postfix-and-infix-notation
*/
package myPackage;
...
public class Calculator extends javax.swing.JFrame {
    ...
    private void addButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addButtonActionPerformed
        ...
        // get position of last char in string
        int length = output.getText().length();
        int position = output.getText().length()-1;
        
        // do not add operator to screen unless it won't be duplicated
        if (!duplicates(output.getText().charAt(position), '+')){
...
// catch out of bounds error
    ...
    private void equalButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_equalButtonActionPerformed
        ...
        if (isBalanced(workingEquation) && balancedOperators(workingEquation)){
            
            String adjEquation = stringPrep(workingEquation);
                    
            output.setText(totalEval((InfixToPostFix((adjEquation)))));
    ...
    public boolean balancedOperators(String equation){
        
        char[] charArray = equation.toCharArray();
        
        int arrayLength = charArray.length;
        int lastPosition = arrayLength -1;
       ...
       for (int i = 0; i < charArray.length;i++){
           ...
           {
            if (charArray[i-1] == '-' && !Character.isDigit(charArray[i+1])){
                return false;
            }
    ...
    // function to ensure no duplicate operators
    public boolean duplicates(char operator1, char operator2){
   ...
   public int getPrecedence(char operator){
    ...
    // prepping string for conversion to postfix, changing brackets and subtraction to negation where necessary
     public String stringPrep(String expression){
      ...
      // send string to char array
      char[] expArray = infix.toCharArray();
      
      // changing out subtraction for negation
      for (int i = 0; i < infix.length(); i++){
      
      // minus check
         if (expArray[i] == '-'){

         // if character before is not null
            if (i == 0){
               expArray[i] = '~';
            }
            ...
            // if character before is not a number or is not a left parenthesis
               if (!Character.isDigit(expArray[i-1])){
      ...
      } // end negation check/change
       
      // convert back to string
      infix = String.valueOf(expArray);
     ...
     // Pseudocode stringly referenced from https://en.wikipedia.org/wiki/Shunting-yard_algorithm
    ...
    // do for each item in queue
      while (!queue.isEmpty()){
         ...
         if (isOperator(token.charAt(0)) && !stack.isEmpty()){
            ...
            // if token is a function and stack is not empty
